...
**Codex installs vendor the same assets (`codex@2`).** `stark install --runtime codex` writes the snapshot next to the skills: `.agents/skills/<name>/SKILL.md` (+ that skill's `references/`), and the bundle's assets root at **`.agents/stark/<bundle>/`** (`tools/`, `standards/`, `prompts/`, `scripts/`, `config.json`). The root is **per bundle, never shared** — `stark-gh` ships its own `config.json` that would clobber the shared snapshot's in a flat namespace. Because the tree differs from a Claude plugin's, the codex target retargets both reference shapes in every rendered body: `${CLAUDE_PLUGIN_ROOT…}` → `${STARK_PLUGIN_ROOT:-$HOME/.agents/stark/<bundle>}` (the `$HOME` fallback assumes a global install; export `STARK_PLUGIN_ROOT` for a project-local one), and `../../{tools,standards,prompts,scripts}/` → `../../stark/<bundle>/…` (on Claude `../../` from `skills/<name>/SKILL.md` **is** the plugin root; on Codex it's `.agents/`, one level short). Vendor roots default off `--catalog`'s parent; override with `--assets-source` / `--plugin-assets`. **Not covered:** `--runtime claude` doesn't vendor (its distribution path is the committed `dist/claude/` tree, which already has them); `--runtime gemini` is `stark-gh` only — the other 26 skills declare `[claude, codex]`.

...
INFRA_JSON="$(STARK_ASSET_ROOT="${STARK_PLUGIN_ROOT:?resolve from this loaded SKILL.md as instructed above}" node --experimental-strip-types "$TOOLS/housekeeping_infra.ts" \
  "${dry_run_args[@]}" --json)"
printf '%s\n' "$INFRA_JSON"
```

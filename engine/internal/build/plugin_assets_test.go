...
// the shared snapshot untouched. This is the regression guard for missing
// plugin tools (gh_cleanup.ts et al.) — the shared snapshot alone never carried
// the plugin-specific tools, so a plugin command crashed with MODULE_NOT_FOUND.
func TestPluginAssetsOverrideSharedSnapshot(t *testing.T) {
	...
	mustWrite(t, filepath.Join(plugins, "stark-ops", "tools", "gh_cleanup.ts"), "export const c = 1\n")
	...
	assertFile(t, out, "dist/claude/stark-ops/tools/gh_cleanup.ts", "export const c = 1\n")
